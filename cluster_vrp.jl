...
for alpha in 0 : 0.05 : 1
    #objective
    @objective(model, Min, sum(sum(x[i, j] * ((1 - alpha) * emissions_matrix_vrp[i, j] + alpha * 100 * attendance_loss_matrix[i,j]) for i in V) for j in V));

    println(now())
    println("Optimize model for alpha = $alpha")
    flush(stdout)

    optimize!(model)

    println(now())
    println("Optimization done for alpha = $alpha")
    flush(stdout)

    x_opt = value.(x);
    df_full = convert_jump_container_to_df(x_opt, dim_names=[:i, :j], value_col=:x)
    df_1 = df_full[df_full.x .== 1, :]
    CSV.write("/home/gbonheur/result_vrp_attend_loss/result_vrp_$alpha.csv", df_1)
    ...
    file = open("/home/gbonheur/result_vrp_attend_loss/results", "a")
    write(file, "$alpha :\t $tot_em \t $tot_attendance \n")
    close(file)
end

# write results to file
CSV.write("/home/gbonheur/result_vrp_attend_loss/results.csv", results)
